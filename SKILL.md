...
3. לחץ **Create bucket** (**חובה לפני יצירת ה-API token**):
...
5. חזור לעמוד R2 הראשי → **Manage R2 API tokens** → **Create Account API token**:
   ...
   - TTL: Forever
6. **שמור מיד את 3 הערכים בלבד** (מוצגים פעם אחת בלבד):
   - Access Key ID
   - Secret Access Key
   - Account ID (מופיע ב-Endpoint כמו `https://XXXXX.r2.cloudflarestorage.com` — ה-XXXXX הוא ה-Account ID)

   ⚠️ יש גם "Token value" בדף — **לא צריך אותו**, רק את 3 הערכים למעלה.

### שלב 3 — שליפת פרטי חיבור Supabase (המשתמש עושה ידנית)

⚠️ **חשוב**: GitHub Actions רץ על רשת IPv4 בלבד. חיבור Direct של Supabase הוא IPv6 — לא יעבוד. חובה להשתמש ב-**Session Pooler**.

1. Supabase Dashboard → לחץ **Connect** למעלה
2. בחר **Session pooler** (לא Direct, לא Transaction pooler)
...
ב-`https://github.com/USER/REPO/settings/secrets/actions` הוסף 7 secrets.

⚠️ **חשוב**: הוסף תחת **Repository secrets** בלבד — לא "Environment secrets" ולא "Organization secrets". אם הוספת למקום הלא נכון, ה-workflow יראה את ה-secrets ריקים בלי שגיאה ברורה.
...
ראה `troubleshooting.md` ליד הסקיל. הבאגים המתועדים:
...
5. Secrets מופיעים ריקים בלוג (הוספה ל-Environment secrets במקום Repository secrets)
6. `Network is unreachable` — שימוש ב-Direct connection (IPv6) במקום Session Pooler
7. `NoSuchBucket` — ה-bucket לא נוצר לפני יצירת ה-API token
...
ה-template ב-`templates/db-backup.yml.template` הוא הקוד הסופי שנבדק על Supabase production (פרויקט פעיל, 13MB DB, 540 הודעות). נתקלנו ב-7 באגים תוך כדי, ואת כולם פתרנו — הפתרונות מטמועים בתבנית.
